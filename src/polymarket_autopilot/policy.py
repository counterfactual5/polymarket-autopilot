...
_DEFAULT_PROJECT = "polymarket-autopilot"
...
def _check_price_range(policy: Policy, ctx: dict[str, Any], res: CheckResult) -> None:
    min_p = policy.extra.get("min_price")
    max_p = policy.extra.get("max_price")
    price = ctx.get("price")
    if price is None:
        return
    try:
    ...
    if min_p is not None:
        try:
            if price_dec < Decimal(str(min_p)):
                res.violations.append(Violation(
                    rule="min_price",
                    message=f"price {price_dec} below minimum {min_p}",
                ))
        except (InvalidOperation, ValueError):
            pass
    if max_p is not None:
        try:
            if price_dec > Decimal(str(max_p)):
                res.violations.append(Violation(
                    rule="max_price",
                    message=f"price {price_dec} above maximum {max_p}",
                ))
        except (InvalidOperation, ValueError):
            pass


def _check_max_position_value(policy: Policy, ctx: dict[str, Any], res: CheckResult) -> None:
    max_val = policy.extra.get("max_position_value")
    if max_val is None:
        return
    position_value = ctx.get("position_value")
    if position_value is None:
        return
    try:
        pv = Decimal(str(position_value))
        mv = Decimal(str(max_val))
    except (InvalidOperation, ValueError):
        return
    if pv > mv:
        res.violations.append(Violation(
            rule="max_position_value",
            message=f"position value {pv} exceeds limit {mv}",
        ))
...
def check_polymarket(
    ...
    """Run shared + Polymarket-specific checks."""
    result = check(policy, context)
    _check_price_range(policy, context, result)
    _check_max_position_value(policy, context, result)
    if result.violations:
        result.allowed = False
    return result
